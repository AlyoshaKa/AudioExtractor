AudioExtractor Readme

This is a GUI for ffmpeg. It's value is to avoid needing to remember specific commands and flags for a cli.

It accepts an MKV as input, and then allows the user to select the audio streams to extract either in wav or flac formats. The user can select either lossless or lossy decoding, and select to output either as multichannel file or seperate, single channel files.
